...
import { ArrowLeft, Plus, UtensilsCrossed, Loader2, Edit, Trash2, FileSpreadsheet, Upload, AlertCircle, Copy } from 'lucide-react';
...
export default function AdminDapurPage() {
  ...
  const [editForm, setEditForm] = useState({ nama: '', alamat: '' });
  ...
  const startEdit = (d: any) => {
    setEditingId(d.id);
    setEditForm({ nama: d.nama, alamat: d.alamat || '' });
  };
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error('Gagal update dapur');
      setEditingId(null);
      loadData();
    } catch (e: any) {
      alert(e.message);
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div className="space-y-2 md:col-span-2">
                  ...
                  <Input placeholder="Alamat lengkap fasilitas dapur" value={form.alamat} onChange={e => setForm({...form, alamat: e.target.value})}/>
                </div>
              </div>
              <div className="pt-2">
      ...
      <div className="pt-10 space-y-6">
        <h3 className="font-bold text-lg text-foreground px-1 mb-2">Daftar Dapur</h3>
        {loading ? (
                ...
                <CardContent className="p-6 flex-1 flex flex-col gap-4">
                  {editingId === d.id ? (
                    <div className="space-y-3">
                      <Input value={editForm.nama} onChange={e => setEditForm({...editForm, nama: e.target.value})} placeholder="Nama Dapur" />
                    ...
                    <>
                      <div className="flex items-start justify-between gap-4">
                        <div className="space-y-1.5 min-w-0">
                          <div className="font-bold text-foreground text-lg truncate" title={d.nama}>{d.nama}</div>
                        ...
                        <div className="flex gap-1 shrink-0 pt-1">
                          <Button variant="ghost" size="icon" className="h-9 w-9 text-muted-foreground" onClick={() => startEdit(d)}>
                            <Edit size={18} />
                          </Button>
                          <Button variant="ghost" size="icon" className="h-9 w-9 text-destructive/80 hover:text-destructive" onClick={() => handleDelete(d.id)}>
                            <Trash2 size={18} />
                          </Button>
                        </div>
                      </div>
